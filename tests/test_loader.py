...
class LoaderTests(unittest.TestCase):
    ...
    def test_select_cases_keeps_global_and_source_cases(self) -> None:
        ...
        selected = select_cases(cases, "brain-client")
        ids = {case.id for case in selected}
        self.assertIn("never-close-foreign-task", ids)  # global
        self.assertIn("client-okf-frontmatter", ids)  # grounded in brain-client
        self.assertNotIn("brain-log-rotation", ids)  # grounded in brain
