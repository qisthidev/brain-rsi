...
from brain_rsi.sources import is_secret_filename, load_registry, redact_secrets  # noqa: E402
...
def migrate(source_id: str, source: Path, destination: Path, *, max_bytes: int, dry_run: bool) -> dict:
...
def main(argv: list[str] | None = None) -> int:
    ...
    status = 0
    for spec in specs:
        source = spec.resolved_legacy_path() or spec.resolved_path()
        if not (source / ".git").exists():
            print(f"[migrate] {spec.id}: {source} is not a git repository, skipping", file=sys.stderr)
            status = 1
            continue
        manifest = migrate(
            spec.id, source, args.destination / spec.id, max_bytes=int(args.max_file_mb * 1024 * 1024), dry_run=args.dry_run
        )
        print(
            f"[migrate] {spec.id}: {manifest['file_count']} files, {manifest['total_bytes'] / 1e6:.1f} MB, "
            f"{manifest['skipped_count']} skipped, {manifest['redacted_count']} redacted, "
